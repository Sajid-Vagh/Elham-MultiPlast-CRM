...
export default function DispatchPage() {
  ...
  const [form, setForm] = useState({ orderId: "", vehicleNumber: "", driverName: "", driverMobile: "", transportCompany: "", lrNumber: "", dispatchDate: "", expectedDeliveryDate: "", dispatchAddress: "", remarks: "" });
      ...
      <Card><CardContent className="p-0">
        <Table>
          <TableHeader><TableRow>
            <TableHead>Dispatch #</TableHead><TableHead>Order</TableHead><TableHead>Customer</TableHead>
                  ...
                  <TableCell>
                    {d.status !== "Delivered" && d.status !== "Cancelled" && (
                      <Select value={d.status} onValueChange={v => updateStatus.mutate({ id: d.id, status: v })}>
                        ...
                        <SelectContent>{["Pending", "Vehicle Assigned", "Loaded", "Dispatched", "In Transit", "Delivered", "Delayed", "Cancelled"].map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                      </Select>
                    )}
                  </TableCell>
                </TableRow>
