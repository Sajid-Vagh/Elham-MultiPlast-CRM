...
export default function Login() {
  ...
  // The First-Admin Setup card is shown on the RIGHT side ONLY while the
  // server reports that bootstrap is possible (no Admin exists yet AND the
  // secure conditions are satisfiable). Once an Admin exists this is false
  // and ONLY the Login card renders. There is no public registration.
  const [bootstrapAvailable, setBootstrapAvailable] = useState<boolean | null>(null);

  // Check if first-admin setup is available
  useEffect(() => {
    ...
    fetch(resolveApiUrl("/api/auth/setup-status"), {
      ...
      .then((data: { adminExists: boolean; bootstrapAvailable?: boolean }) => {
        setBootstrapAvailable(data.bootstrapAvailable === true);
      })
      .catch(() => {
        // If we can't check, hide the setup card and show login only
  ...
  // Show loading while checking setup availability
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
      <div className={`w-full ${bootstrapAvailable ? "max-w-4xl" : "max-w-sm"} flex flex-col ${bootstrapAvailable ? "md:flex-row" : ""} items-stretch justify-center gap-6`}>
        {/* LEFT — Login */}
        <Card className={`w-full ${bootstrapAvailable ? "md:w-80" : "max-w-sm"} shadow-xl border-primary/10 self-center`}>
        ...
        {/* RIGHT — First Admin Setup (ONLY while bootstrap is possible) */}
        {bootstrapAvailable && (
