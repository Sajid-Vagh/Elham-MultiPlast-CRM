                ...
                <p className="text-xs text-muted-foreground">
                  {item.stage ? (
                    <>
                      ...
                      {item.piStatus && item.piStatus !== "No PI" && <> · PI: {item.piStatus}{item.piNumber ? ` (${item.piNumber})` : ""}</>}
                    </>
                  ) : (
                    [item.companyName, item.mobile, item.status, item.category].filter(Boolean).join(" - ")
                  )}
                </p>
              </div>
            ))}
