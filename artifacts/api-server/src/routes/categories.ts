...
router.get("/categories/:category/contacts/search", async (req, res) => {
  try {
    ...
    if (q) {
      const s = `%${q}%`;
      schConditions.push(
        sql`(${contactsTable.name} ILIKE ${s} OR ${contactsTable.mobile} ILIKE ${s} OR ${contactsTable.companyName} ILIKE ${s} OR ${contactsTable.city} ILIKE ${s} OR ${contactsTable.state} ILIKE ${s} OR ${contactsTable.customerCode} ILIKE ${s})`
      );
    }
